...
def run_eval_b(angle_deg, duration=10.0, show_animation=True, save=True):
    print(f"Evaluation B (ideal): Recovery from {angle_deg}°")
    print("  Using true state (no observer)")
    ...
    results = simulate(initial_state, t_span, controller=controller, observer=None)
    ...
    if save:
        ...
        filename = f"eval_b_ideal_{abs(angle_deg)}deg.txt"
        metadata = {
            'Evaluation': 'B (Recovery)',
            'Mode': 'Ideal (true state)',
    ...
    if show_animation:
        title = f"Eval B (ideal): Recovery from {angle_deg}°"
...
def main():
    parser = argparse.ArgumentParser(description='Cart-Pendulum Control (Ideal - True State)')
